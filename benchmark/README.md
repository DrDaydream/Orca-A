...
Orca-A note: when `faults > 0`, all nodes remain online. The primary command propagates `ORCA_FAULTS` and enables deterministic Rule 1/2/3 adversarial scheduling; `faults` no longer means that the last nodes are omitted.
